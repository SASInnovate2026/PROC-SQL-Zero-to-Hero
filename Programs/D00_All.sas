/* libname sqw "/export/viya/homes/carleighjo.crabtree@sas.com/casuser/SQLDemo/choc_enterprise"; */
/* libname out "/export/viya/homes/carleighjo.crabtree@sas.com/casuser/SQLDemo/choc_output"; */
...
select customer_name, email
	from sqw.choc_enterprise_customer
	/***/ loyalty_program="Non-Member" and email is not null;
quit;
...
proc sql;
select /***/ as TotalNonMembers format=comma16.
	from sqw.choc_enterprise_customer
...
select customer_name,	
	...
	/***/ as Customer_Names_Cleaned label="Customer Name",
	email, bday_month, age, customer_self_description, loyalty_program, customer_rk
	from out.customers;
quit;
...
select *
	...
	where libname="SQW" and /***/ like "CHOC%";
quit;
...
select libname, memname, name, type, length, label
	from dictionary.columns
	where libname="SQW" and memname like "CHOC%" and /***/="item_rk";
quit;
...
select *
	from sqw.choc_enterprise_orders as o 
		/***/ sqw.choc_enterprise_item as i
	on o.item_rk = i.item_rk 
		/***/ out.customers_cleaned as c
...
proc sql;
select /***/item_qty as AvgItemsOrdered
...
select count(customer_names_cleaned) as SugarFreeOrders format=comma16.
	from out.orditemscust
	where category="Sugar Free" and total_line_item_sale_amt> /***/;
title;
quit;
...
select product_line, category, /***/total_line_item_sale_amt as CategoryProfit format=dollar25.
	from out.orditemscust
...
proc sql;
select /***/Assorted as HighestAssortedOrders format= comma16., 
       /***/Assorted as LowestAssortedOrders format= comma16.,
	   /***/Assorted as AverageAssortedOrders format= comma16.
...
select state_region_nm label= "State Name",
	   max(/***/) as HighestOrders format= comma16., 
       min(/***/) as LowestOrders format= comma16.,
	   mean(/***/) as AverageOrders format= comma16.
	from out.spl_transposed;	
quit;
...
select state_region_nm label= "State Name", customer_names_cleaned,
		find(loyalty_program, "Chocolate Club Member", "i")/***/ as ChocolateClubMember format= comma16.,
		find(loyalty_program, "Chocolate Club Member", "i")/***/ as NonMember format= comma16.
	from out.orditemscust
	where state_region_nm is not null;
quit;
...
proc sql;
select state_region_nm label= "State Name",
		/***/find(loyalty_program, "Chocolate Club Member", "i")>0 as ChocolateClubMembers format= comma16.,
		/***/find(loyalty_program, "Chocolate Club Member", "i")=0 as NonMembers format= comma16.
	from out.orditemscust
	where state_region_nm is not null
